...
`orph_datasette_command()` generates that second command, so it cannot drift
from what the code expects. Pass `ui = FALSE` for a reader with no upload page.

### Two YAML files, and they are not interchangeable

Both are generated by `orph_write_datasette_config()` from the bundle and the
permission model, so they cannot disagree with either. Datasette reads them
...
plugins:
  orpheus-datasette:
    ...
    token:
      $env: ORPHEUS_API_TOKEN  # named, not written: regenerating commits no secret
    # actor_tokens:            # per-actor instead, so amendments name the real
    #   "nuala@dept.ie":       # person rather than one shared identity
    #     $env: ORPHEUS_TOKEN_NUALA
```

Because the file is generated and committed, it cannot name a host that is only
right in one deployment. `ORPHEUS_API_URL` overrides `api_url`, which is how the
compose stack points the UI at `http://api:8000` without a second copy of the
file.
...
in `inst/datasette/datasette.yml`. Regenerate it with
`orph_write_datasette_config()` after any change to the permission model.
