...
def build_config(bundle: dict, database_name: str = "orpheus",
    ...
    return {
        ...
        # even consulted.
        "allow": {"id": "*"},
        # The plugin calls the core in-process, so there is no base URL and no
        # token to hand it: it is already inside the process that holds the
        # database. What it needs to be told is which database, and where
        # ingest should put originals.
...
def write_config(path: str | Path = "config/datasette.yml",
    ...
    header = (
        ...
        f"#\n#   datasette serve data/{database_name}.sqlite \\\n"
    ...
    path.write_text(header
                    + _yaml(build_config(bundle, database_name, storage_root,
                                         max_file_size))
                    + "\n" + permissions)
